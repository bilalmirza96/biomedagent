...
- **Always read FIRST** `__PLUGIN_DIR__/skills/biomedagent/references/memory-log.json` — the self-evolving memory log. Check
...
- **Always read** `__PLUGIN_DIR__/skills/biomedagent/references/tool-catalog.md` — contains all 65 biomedical tools with their
  inputs, outputs, and descriptions. You need this to score and select tools.
- **Read when designing workflows** `__PLUGIN_DIR__/skills/biomedagent/references/workflow-examples.md` — contains example
  workflows for common analysis patterns (omics, ML, statistics, visualization).
- **Read when you need prompt patterns** `__PLUGIN_DIR__/skills/biomedagent/references/prompts-reference.md` — contains the
...
Read `__PLUGIN_DIR__/skills/biomedagent/references/tool-catalog.md` to get the full list of 65 biomedical tools. For each tool,
...
Read `__PLUGIN_DIR__/skills/biomedagent/references/workflow-examples.md` for concrete examples of well-designed workflows.
...
JSON-based memory log at `__PLUGIN_DIR__/skills/biomedagent/references/memory-log.json`.
...
`__PLUGIN_DIR__/skills/biomedagent/references/memory-log.json` and search for entries matching the current task:
...
were produced, and the user is satisfied — append a new entry to `__PLUGIN_DIR__/skills/biomedagent/references/memory-log.json`:
...
8. **Check memory before working, write memory after succeeding.** Always read
   `__PLUGIN_DIR__/skills/biomedagent/references/memory-log.json` before starting Phase 1. If a matching pattern exists,
